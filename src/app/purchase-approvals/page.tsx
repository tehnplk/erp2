...
const getPlanFlagBadgeClass = (planFlag?: string | null) => {
  if (planFlag === 'นอกแผน') {
...
function PurchaseApprovalsPageContent() {
  ...
  const formatQuantity = (value?: number | string) => {
    ...
    return Math.trunc(parsed).toLocaleString('th-TH');
  };
          ...
          (item, index) =>
            new TableRow({
              children: [
                ...
                getPlanFlagLabel(item.usage_plan_flag),
                ...
                new TableCell({
                  ...
                  children: [
                    new Paragraph({
                      alignment:
                        cellIndex === 0 || cellIndex === 2 || cellIndex === 3 || cellIndex === 4 || cellIndex === 5 || cellIndex === 6 || cellIndex === 7
                          ? AlignmentType.CENTER
                          : AlignmentType.LEFT,
                      children: [createDocxTextRun({
                        text,
                        size: 30,
                      })],
                    }),
                            ...
                            <table className="min-w-full divide-y divide-gray-200 border border-gray-200">
                              <thead className="bg-gray-100">
                                <tr>
                                  ...
                                  <th className="px-3 py-2 text-left text-[10px] font-medium text-gray-600 uppercase tracking-wider">แผน</th>
                                </tr>
                              </thead>
                              <tbody className="bg-white divide-y divide-gray-100">
                                {group.sub_items?.map((item, subIndex) => (
                                  <tr key={item.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-3 py-2 text-xs">
                                      <span
                                        className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] font-medium ${getPlanFlagBadgeClass(item.usage_plan_flag)}`}
                                      >
                                        {getPlanFlagLabel(item.usage_plan_flag)}
                                      </span>
                                    </td>
                  ...
                  <tbody>
                    {(documentPreview.sub_items || []).map((item, index) => (
                      <tr key={item.id}>
                        ...
                        <td className="border border-gray-500 px-2 py-2 text-center align-top">{getPlanFlagLabel(item.usage_plan_flag)}</td>
