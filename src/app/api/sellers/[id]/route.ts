...
export async function GET(
  ...
  try {
    ...
    const result = await pgQuery(
      'SELECT id, code, prefix, name, business, address, phone, fax, mobile, COALESCE(category_code_sale, ARRAY[]::text[]) AS category_code_sale, is_active FROM public.seller WHERE id = $1 AND is_active = true LIMIT 1',
      [id]
    );
...
export async function PUT(
  ...
  try {
    ...
    const columnMap: Record<string, string> = {
      ...
      fax: 'fax',
      mobile: 'mobile',
      category_code_sale: 'category_code_sale',
      is_active: 'is_active',
    };
    ...
    if (assignments.length === 0) {
      const unchangedResult = await pgQuery('SELECT id, code, prefix, name, business, address, phone, fax, mobile, COALESCE(category_code_sale, ARRAY[]::text[]) AS category_code_sale, is_active FROM public.seller WHERE id = $1 LIMIT 1', [id]);
